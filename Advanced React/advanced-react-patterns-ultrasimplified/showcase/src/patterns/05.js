...
const MediumClap = ({children, values=null, onClap, style:userStyle={}, className}) => {
    ...
    React.useEffect(() => {
        if (!componentJustMount.current && !isControlled) {
            onClap && onClap(clapState)
        }
        componentJustMount.current = false
    }, [clapState, values, onClap]);
    ...
    const memoizedValue = React.useMemo(() => ({...getState(), setAllRef}), [getState, setAllRef])
...
const MAX_CLAP_VAL = 50;
const Usage = () => {
    const [state, setState] = React.useState(initState);
    const handleClap = () => {
        setState(({count, totalCount}) =>({
            count:Math.min(count+1, MAX_CLAP_VAL),
            totalCount: count<MAX_CLAP_VAL? totalCount+1:totalCount,
            isClicked: true,
        }))
    }
    return <div>
        <div style={{width:"100%"}}>
            ...
            <MediumClap onClap={handleClap} values={state} style={{border:"1px solid blue"}}  className={userCustomStyle.clap} >
                <MediumClap.Icon />
        ...
        <span>{`You have clapped ${state.count} times`}</span>
    </div>
